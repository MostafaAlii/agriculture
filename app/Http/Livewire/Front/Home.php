...
class Home extends Component
{
    ...
    {
        $data['newProducts'] = Product::where('status',1)->where('stock',1)->where('qty','>',0)->latest()->limit(12)->get();
        $data['popProducts'] = Product::where('status',1)->where('stock',1)->where('qty','>',0)->inRandomOrder()->get()->take(6);
        $data['saleProducts'] = Product::where('status',1)->where('special_price','>',0)
                                 ->where('stock','1')
                                 ->latest()->get()->take(4);
