        ...
        <div class="modal animated flipInY text-left"  tabindex="-1" role="dialog" aria-hidden="true" id="delete{{ $id }}">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <form action="{{ route('Trees.destroy', encrypt($id)) }}" class="my-1 my-xl-0" method="post" style="display: inline-block;">
                        @csrf
                        @method('delete')
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control"
                                       ...
                                       placeholder="{{ trans('Admin\trees.enter_tree_name_placeholder') }}"
                                       value="{{\App\Models\Tree::findorFail($id)->first()->name}}"/>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="tree_type_id" class="select2 form-control">
                                    <optgroup label="{{ trans('Admin\trees.please_choose_tree') }}">
                                        @php
                                            $treeTypes = treeTypes();
                                        @endphp
                                        ...
                                        @foreach($treeTypes as $treeType)
                                            <option value="{{$treeType->id }}"
                                                    {{$treeType->id ==\App\Models\Tree::findorFail($id)->first()->tree_type_id?'selected':''}}>
                                                {{$treeType->tree_type}}
                                            </option>
                                        @endforeach
                                    </optgroup>
...
{{-- modal bulk delete --}}
<form action="{{ route('trees.bulk_delete','ids') }}" class="my-1 my-xl-0" method="post" style="display: inline-block;">
    @csrf
    @method('delete')
    <div class="col-lg-4 col-md-6 col-sm-12">
        <div class="form-group">
            ...
            <div class="modal animated flipInY text-left" id="bulkdelete" tabindex="-1" role="dialog" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn grey btn-outline-secondary" data-dismiss="modal"> {{ __('Admin\trees.cancel') }}</button>
                            <button type="submit" class="btn btn-outline-primary"> {{ __('Admin\trees.delete') }}</button>
                        </div>
                    </div>
                </div>
